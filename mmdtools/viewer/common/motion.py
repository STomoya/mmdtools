...
class Motion:
    ...
    def __init__(self, model: Model, motion_data: vmd.VMDFile) -> None:
        ...
        for name, key_frames in motion_data.bone_animation.items():
            ...
            for current, next in zip(self.motions_key_frames[name], self.motions_key_frames[name][1:], strict=False):
    ...
    def finished(self) -> bool:
        ...
        return all(key_frame.next_frame is None for key_frame in self.current_key_frames.values())

    def step(self) -> None:
        """step frame."""
        for name in self.current_key_frames:
