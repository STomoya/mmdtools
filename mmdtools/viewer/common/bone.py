...
class Bone:
    ...
    def __init__(self, index: int, bone: mmd.Bone) -> None:
        ...
        self.name_en: str = bone.name_en
        self.level: int = bone.level
        ...
        self.is_controllable: bool = bone.is_controllable
        self.is_rotatable: bool = bone.is_rotation
        self.is_movable: bool = bone.is_movable

        # IK
        self.is_ik: bool = bone.is_ik
        ...
        # follow paranet bone
        self.grant_is_translation: bool = False
        ...
        # initialized flags.
        self._is_parent_initialized: bool = False
        self._is_ik_initialized: bool = False
        self._is_additional_trans_initialized: bool = False

        # raw bone data
        self._raw_data: mmd.Bone = bone
    ...
    def set_parent_bone(self, parent: Bone|None) -> None:
        """Sets the parent bone alongside with some matrix attributes according to the input.
    ...
    def set_ik_bones(self, effect_bone: Bone|None, links: list[Bone]| None) -> None:
        """Sets the effected and linked bone object for IK. The input must be an instantiated `Bone` class object.
    ...
    def set_grant_transform_parent_bone(self, bone: Bone, rate: float, is_translation: bool) -> None:
        """set configuration for bones following its parent. `bone` argument must be an instantiated `Bone` class object.
    ...
    def reset_delta(self) -> None:
        """resets the `delta_matrix` for next motion.
        """
    ...
    def set_global_position(self, position: np.ndarray) -> None:
        """set position to the `global_matrix`, given a translation vector of size 3.
    ...
    def rotatable_control(bone: Bone, axis: np.ndarray, rotation: float) -> None:
        """defines the rotation control function for bones.
